...
export default function MaterialDetailClient({
    ...
    const [paymentPending, setPaymentPending] =
        useState(false)
    const [coverImgFailed, setCoverImgFailed] =
        useState(false)
    ...
    const materialConfig = getMaterialTypeConfig(
    ...
    const hasGradient = (
        cfg: any
    ): cfg is { gradient: { from: string; to: string } } =>
        !!cfg &&
        typeof cfg === "object" &&
        "gradient" in cfg &&
        cfg.gradient &&
        typeof cfg.gradient.from === "string" &&
        typeof cfg.gradient.to === "string"

    const grad = hasGradient(materialConfig)
        ? materialConfig.gradient
        : { from: "#e6f9ee", to: "#cfeede" }
    const typeLabel =
        materialConfig?.label ?? material.materialType
    const teacherHandle =
        material.teacher?.name ??
        material.teacher?.email?.split("@")[0] ??
        "Unknown"
    const isVerified =
        !!material.teacher?.teacherProfile?.isActive
    const isTeacher = user?.id === material.teacher?.id

    // Confetti celebration for successful purchase
    const triggerConfetti = useCallback(() => {
        ...
        // Return a cleanup function to cancel pending animation frames
        return () => {
    ...
    const handleDownload = useCallback(async () => {
        ...
        try {
            ...
            const res = await fetch(
                `/api/pdf/download?pdfId=${material.id}`,
                {
    ...
    useEffect(() => {
        ...
        const interval = setInterval(async () => {
            ...
            if (elapsed >= MAX_POLL_SECONDS) {
                ...
                toast.error(
                    "Payment verification timed out. If you paid, please refresh the page."
                )
                return
            }
            ...
            try {
                const res = await fetch(
                    `/api/purchase/check?pdfId=${material.id}&_t=${Date.now()}`,
                    {
                        cache: "no-store"
                    }
                )
                ...
                if (data.purchased) {
                    ...
                    triggerConfetti()
                    toast.success(
                        "Payment successful! You can now download the material."
                    )
    ...
    const updatePhoneProfile = async (
        phoneNumber: string
    ) => {
        try {
    ...
    function onBuyClick() {
        if (!user) {
            ...
            router.push(
                `/login?callbackUrl=${encodeURIComponent(`/marketplace/${material.slug ?? material.id}?autoBuy=1`)}`
            )
            return
        }
    ...
    const previewPdfSrc = `/api/pdf/preview?pdfId=${encodeURIComponent(material.id)}&mode=${purchased ? "full" : "preview"}`
        ...
        <div className="min-h-screen bg-[#f5faf7]">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2 space-y-5">
                        ...
                        <div className="bg-white rounded-2xl border-2 border-gray-100 shadow-sm p-4">
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                                <StatChip
                                    value={material._count.purchases.toString()}
                                    label="Sales"
                                    color="emerald"
                                    icon={
                                        <svg
                                ...
                                />
                                <StatChip
                                    value={material._count.downloads.toString()}
                                    label="Downloads"
                                    color="sky"
                                    icon={
                                        <svg
                                ...
                                />
                                <StatChip
                                    value={material._count.materialViews.toString()}
                                    label="Views"
                                    color="violet"
                                    icon={
                                        <svg
                        ...
                        <div className="bg-white rounded-2xl border-2 border-gray-100 shadow-sm p-5">
                            ...
                            <div className="flex items-center gap-3">
                                <div
                                    className="w-12 h-12 rounded-2xl flex items-center justify-center text-white font-extrabold text-xl shrink-0"
                                    style={{
                                        background: `linear-gradient(135deg, ${grad.from} 0%, ${grad.to} 100%)`
                                    }}
                                >
                    ...
                    <div className="lg:col-span-1">
                        <button
                            ...
                            className="w-full px-6 py-3 bg-white text-[#006832] border-2 border-[#d1e8dc] rounded-md font-semibold hover:bg-[#f0faf5] transition-colors flex items-center justify-center gap-2 mb-3"
                        >
                            <svg
                                className="w-5 h-5"
                                fill="none"
                        ...
                        {paymentPending && (
                            <div className="mb-4 p-5 bg-primary/5 border-2 border-primary/20 rounded-lg text-center">
                                ...
                                <p className="text-sm text-muted-foreground mb-3">
                                    ...
                                    M-Pesa prompt and enter
                                    your PIN
                                </p>
                                <p className="text-xs text-muted-foreground">
                                    This may take up to 90
                                    seconds...
                                </p>
                            </div>
                        )}
                        ...
                        <div className="pt-3 border-t border-gray-100 space-y-2.5">
                            {[
                                ...
                                >
                                    <svg
                                        className="w-3.5 h-3.5 text-[#008c43] shrink-0"
                                        fill="none"
                                        viewBox="0 0 24 24"
                                        stroke="currentColor"
                                    >
            ...
            {showPreviewModal &&
                createPortal(
                    ...
                    >
                        <div
                            className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[92vh] overflow-hidden animate-in fade-in-0 zoom-in-95 duration-200"
                            onClick={e =>
                                e.stopPropagation()
                            }
                        >
                            <div className="px-5 py-4 border-b border-gray-100 flex items-start justify-between gap-4">
                                <div>
                                    <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-[#008c43]">
                                        Material Preview
                                    </p>
                                    ...
                                    <p className="text-xs text-gray-500 mt-1">
                                        {purchased
                                            ? "Full material unlocked. Scroll to read all pages."
                                            : "Showing only page 1 with a locked section. Buy to unlock the complete material."}
                                    </p>
                                </div>
                                <button
                                    onClick={() =>
                            ...
                            <div className="p-4 sm:p-5 overflow-y-auto max-h-[calc(92vh-94px)]">
                                {purchased ? (
                                    <div className="rounded-xl border border-gray-200 bg-gray-100 overflow-hidden h-[70vh]">
                                        <iframe
                                            key={`${material.id}-full-preview`}
                                            src={`${previewPdfSrc}#toolbar=0&navpanes=0&scrollbar=1`}
                                            title={`Full material for ${material.title}`}
                                            className="w-full h-full"
                                        />
                                    </div>
                                ) : (
                                    <>
                                        <div className="relative mx-auto w-full max-w-2xl aspect-[1/1.4142] rounded-xl overflow-hidden border border-gray-200 bg-white shadow-sm">
                                            <iframe
                                                key={`${material.id}-locked-preview`}
                                                src={`${previewPdfSrc}#toolbar=0&navpanes=0&scrollbar=0&page=1&zoom=page-fit`}
                                                title={`Locked preview for ${material.title}`}
                                                className="absolute inset-0 w-full h-full"
                                            />

                                            <div className="pointer-events-none absolute left-3 top-3 rounded-lg bg-white/80 border border-white px-2.5 py-1 text-[10px] font-bold tracking-[0.08em] text-[#006832]">
                                                SOMOVIBE
                                                PREVIEW
                                            </div>
                                            <div className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 rotate-[-20deg] text-4xl sm:text-5xl font-black text-white/45 tracking-[0.18em] select-none">
                                                Somovibe
                                            </div>

                                            <div className="pointer-events-none absolute inset-x-0 bottom-0 h-1/4 bg-white/75 backdrop-blur-md border-t border-white/70 flex items-center justify-center px-4">
                                        ...
                                        <div className="mt-4 flex flex-col sm:flex-row gap-3">
                                            <button
                                                ...
                                                className="flex-1 px-5 py-2.5 text-white rounded-lg font-semibold transition-opacity disabled:opacity-50"
                                                style={{
                                                    background:
                                                        "linear-gradient(135deg, #006832 0%, #008c43 100%)"
                                                }}
                                            >
                                                {purchasing ||
                                                paymentPending
                                                    ? "Processing Purchase..."
                                                    : "Buy Now to Unlock Full Material"}
                                            </button>
                                            <button
                                                onClick={() =>
                                                    setShowPreviewModal(
                                                ...
                                                className="sm:w-auto px-5 py-2.5 border border-gray-300 rounded-lg text-gray-700 font-semibold hover:bg-gray-50 transition-colors"
                                            >
                                                Close
                                            </button>
                                        </div>
                    ...
                    >
                        ...
                        >
                            ...
                            <div
                                ...
                                style={{
                                    background:
                                        "linear-gradient(135deg, #003318 0%, #006832 50%, #008c43 100%)"
                                }}
                            >
                                <button
                                    onClick={() =>
                                ...
                                <h3 className="text-white text-lg font-extrabold pr-8 leading-tight line-clamp-2">
                                    {material.title}
                                </h3>
...
function StatChip({
    ...
    return (
        <div className="text-center">
            <div
                className={`w-9 h-9 rounded-xl ${colors[color]} flex items-center justify-center mx-auto mb-1.5`}
            >
                {icon}
            </div>
    ...
    return (
        ...
        <div className="mt-10 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
            <div className="flex items-end justify-between mb-4">
                <div>
                    <p className="text-xs font-bold text-[#008c43] uppercase tracking-wider mb-0.5">
                        {subtitle}
                    </p>
                ...
                <Link
                    href="/marketplace"
                    className="text-sm text-[#008c43] font-semibold hover:underline"
                >
                    View all →
                </Link>
            </div>
