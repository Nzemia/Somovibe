...
export async function POST(req: Request) {
    try {
        ...
        if (resultCode === 0) {
            ...
            const verificationFee = payment.amount; // KES 100

            // Update payment status, activate teacher, and credit admin wallet in a transaction
            await prisma.$transaction(async (tx) => {
                ...
                // Credit platform admin wallet with verification fee
                if (platformAdminId) {
                    const { creditWalletTx } = await import("@/lib/wallet");
                    await creditWalletTx(tx, platformAdminId, verificationFee);
                }
            });
