...
export async function DELETE(request: Request) {
  try {
    ...
    if (material.teacherId !== user.id) {
      return NextResponse.json({ error: "Unauthorized: You do not own this material" }, { status: 403 });
    }
    ...
    await prisma.$transaction([
      prisma.download.deleteMany({ where: { pdfId: id } }),
      prisma.purchase.deleteMany({ where: { pdfId: id } }),
