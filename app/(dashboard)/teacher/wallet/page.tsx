...
export default async function WalletPage() {
    ...
    const totalEarnings = materials.reduce(
        (s, m) =>
            s +
            m.purchases.length * Math.floor(m.price * 0.75),
        0
    )
    const totalSales = materials.reduce(
        (s, m) => s + m.purchases.length,
        0
    )
    ...
    const heroBannerStats = [
        {
            ...
            sub: "75% of sales",
            gradient: "from-[#008c43] to-[#004d25]",
            shadow: "shadow-green-900/30",
            light: "text-green-200",
            lighter: "text-green-300",
            icon: (
                <svg
                    className="w-5 h-5"
                    fill="none"
        ...
        {
            label: "Total Credits",
            value: `KES ${totalCredits.toLocaleString()}`,
            sub: "money received",
            gradient: "from-sky-500 to-blue-600",
            shadow: "shadow-blue-900/30",
            light: "text-sky-200",
            lighter: "text-sky-300",
            icon: (
                <svg
                    className="w-5 h-5"
                    fill="none"
    ...
    return (
        <div className="max-w-5xl mx-auto px-3 sm:px-6 lg:px-8 py-6 sm:py-8 pb-24 sm:pb-10">
            ...
            <div
                ...
                style={{
                    background:
                        "linear-gradient(135deg, #003318 0%, #006832 45%, #008c43 80%, #00b856 100%)"
                }}
            >
                {/* Grid texture */}
                <div
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
                        {heroBannerStats.map(s => (
                            ...
                            >
                                ...
                                <p
                                    className={`${s.light} text-xs font-semibold mt-1`}
                                >
                                    {s.label}
                                </p>
                                <p
                                    className={`${s.lighter} text-xs mt-0.5`}
                                >
                                    {s.sub}
                                </p>
                            </div>
                    ...
                    <div className="divide-y divide-gray-50">
                        {transactions.map(tx => {
                            ...
                            return (
                                <div
                                    key={tx.id}
                                    className="flex items-center justify-between px-5 sm:px-6 py-4 hover:bg-gray-50/60 transition-colors"
                                >
                                    <div className="flex items-center gap-3">
                                        <div
                                            className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${isCredit ? "bg-emerald-50 text-emerald-600" : "bg-amber-50 text-amber-600"}`}
                                        >
                                            {isCredit ? (
                                                <svg
                                                    className="w-4 h-4"
                                    ...
                                    <div className="text-right">
                                        <p
                                            className={`text-sm font-extrabold ${isCredit ? "text-emerald-600" : "text-amber-600"}`}
                                        >
                                            {isCredit
                                                ? "+"
                                                : "−"}
                                        ...
                                        <span className="text-xs bg-[#f0faf5] text-[#006832] font-semibold px-2 py-0.5 rounded-full">
                                            Completed
                                        </span>
                                    </div>
                                </div>
