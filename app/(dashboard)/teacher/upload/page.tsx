...
function SectionLabel({ num, label, done }: { num: number; label: string; done?: boolean }) {
    return (
        <div className="flex items-center gap-3 mb-4">
            <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-extrabold shrink-0 transition-colors ${done ? "bg-[#008c43] text-white" : "bg-[#f0faf5] text-[#008c43] border-2 border-[#008c43]"
                }`}>
                {done ? (
...
export default function UploadPage() {
    ...
    const handleFile = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (f.size > 10 * 1024 * 1024) {
            ...
            toast.error(`File size is ${sizeMB}MB. Maximum allowed size is 10MB`);
            setFile(null);
            return;
        }
        setFile(f);
    ...
    const handleThumb = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!f.type.startsWith("image/")) {
            toast.error("Cover image must be an image file (PNG, JPG, WEBP)");
            setThumbnail(null);
            return;
        }
        if (f.size > 5 * 1024 * 1024) {
            const sizeMB = (f.size / 1024 / 1024).toFixed(1);
            toast.error(`Cover image size is ${sizeMB}MB. Maximum allowed size is 5MB`);
            setThumbnail(null);
            return;
        }
        setThumbnail(f);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (file.size > 10 * 1024 * 1024) {
            ...
            toast.error(`File size is ${sizeMB}MB. Maximum allowed size is 10MB`);
            scrollAndHighlight("file");
            return;
        }
        if (thumbnail && thumbnail.size > 5 * 1024 * 1024) {
            const sizeMB = (thumbnail.size / 1024 / 1024).toFixed(1);
            toast.error(`Cover image size is ${sizeMB}MB. Maximum allowed size is 5MB`);
            return;
        }
        ...
        try {
            ...
            if (!contentType || !contentType.includes("application/json")) {
                if (res.status === 413) {
                    toast.error("File is too large. Material must not exceed 10MB and cover image must not exceed 5MB");
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        {MATERIAL_TYPES.map((t) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`group flex flex-col items-center justify-center gap-2 p-4 rounded-2xl border-2 transition-all text-center ${active
                ...
                <div ref={sectionRefs.file}
                    ...
                    <label htmlFor="file-upload" onClick={handleFileAreaClick}
                        className={`flex flex-col items-center justify-center gap-3 border-2 border-dashed rounded-2xl py-10 px-6 transition-all cursor-pointer ${file
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 sm:p-6">
                    ...
                    <label htmlFor="thumb-upload"
                        className={`flex flex-col items-center justify-center gap-3 border-2 border-dashed rounded-2xl py-8 px-6 transition-all cursor-pointer ${thumbnailPreview ? "border-[#008c43] bg-[#f0faf5]" : "border-gray-200 hover:border-[#008c43]/50 hover:bg-[#f8fdfb]"
                            }`}>
                        {thumbnailPreview ? (
                            <>
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 sm:p-6">
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-5">
                        {[
                            ...
                            <div key={item.label} className={`flex items-center gap-2 text-xs font-semibold px-3 py-2 rounded-xl ${item.done ? "bg-[#f0faf5] text-[#006832]" : "bg-gray-50 text-gray-400"
                                }`}>
                                <div className={`w-4 h-4 rounded-full flex items-center justify-center shrink-0 ${item.done ? "bg-[#008c43]" : "bg-gray-200"
                                    }`}>
                                    {item.done ? (
