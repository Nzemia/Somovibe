...
/* ── Subject synonyms → canonical name ────────────────────
   To avoid accidental duplicate keys (which cause build errors), we
   maintain the list as an array of tuples and then fold it into a
   record with a runtime assertion. This also makes it easy to detect
   typos in the synonyms themselves during development. */

const RAW_SUBJECT_SYNONYMS: [string, string][] = [
  // Mathematics
  ["math", "Mathematics"],
  ["maths", "Mathematics"],
  ["mth", "Mathematics"],
  ["mths", "Mathematics"],
  // duplicate removed; earlier build failure stemmed from the line above
...
// Fold into map, complaining if duplicates are found
const SUBJECT_SYNONYMS: Record<string, string> = (() => {
  const m: Record<string, string> = {};
  for (const [key, val] of RAW_SUBJECT_SYNONYMS) {
    if (key in m) {
      throw new Error(`duplicate synonym key detected: ${key}`);
    }
    m[key] = val;
  }
  return m;
