...
import { computeScore } from "./marketplaceScore";
  ...
  async ({
    ...
    const pdfs = await prisma.pdf.findMany({
      ...
      select: {
        ...
        _count: {
          select: { purchases: true, materialViews: true },
        },
        reviews: {
          select: { rating: true },
        },
    ...
    let items = pdfs.slice(0, take);

    // For trending sort, compute composite score and re-sort
    if (sort === "trending") {
      items = items
        .map((item) => ({ ...item, _score: computeScore(item) }))
