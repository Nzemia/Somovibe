...
const slides: Slide[] = [
  {
    ...
    primaryCta: { label: "Start Earning Today", href: "/teacher-register" },
    secondaryCta: { label: "See How It Works", href: "#how-it-works" },
    bgGradient: "from-[#001a0d] via-[#004d25] to-[#008c43]",
    accentGradient: "from-[#008c43] to-[#00b856]",
    stats: [
      { value: "75%", label: "Commission Rate" },
      { value: "500+", label: "Active Teachers" },
      { value: "KES 100", label: "One-time Verification" },
    ],
    icon: (
      <svg viewBox="0 0 64 64" className="w-full h-full" fill="none">
        <circle cx="32" cy="32" r="30" fill="rgba(255,255,255,0.08)" />
        <path d="M20 44V24l12-8 12 8v20" stroke="white" strokeWidth="2.5" strokeLinejoin="round" />
        <rect x="26" y="32" width="12" height="12" rx="1" stroke="white" strokeWidth="2.5" />
        <path d="M14 44h36" stroke="white" strokeWidth="2.5" strokeLinecap="round" />
        <circle cx="44" cy="20" r="6" fill="rgba(0,184,86,0.3)" stroke="white" strokeWidth="2" />
  ...
  },
  {
    ...
    primaryCta: { label: "Browse Materials", href: "/marketplace" },
    secondaryCta: { label: "Create Free Account", href: "/register" },
    bgGradient: "from-[#002914] via-[#006832] to-[#00a854]",
    ...
    icon: (
      <svg viewBox="0 0 64 64" className="w-full h-full" fill="none">
        <circle cx="32" cy="32" r="30" fill="rgba(255,255,255,0.08)" />
        <rect x="14" y="16" width="22" height="28" rx="2" stroke="white" strokeWidth="2.5" />
        <path d="M19 23h12M19 28h12M19 33h8" stroke="white" strokeWidth="2" strokeLinecap="round" />
        <rect x="28" y="22" width="22" height="28" rx="2" fill="rgba(0,184,86,0.2)" stroke="white" strokeWidth="2.5" />
        <path d="M33 29h12M33 34h12M33 39h8" stroke="white" strokeWidth="2" strokeLinecap="round" />
      </svg>
    ),
  },
];
...
export default function HeroSlider() {
  ...
  const animateIn = useCallback(async () => {
    ...
    const elements = contentRef.current.querySelectorAll<HTMLElement>(".hero-anim");
    try {
      const { animate, stagger, set } = await import("animejs");
            ...
            <div className="hero-anim hidden lg:flex items-center justify-center">
              <div className="relative">
                ...
                <div className="relative w-72 h-72 xl:w-80 xl:h-80 rounded-full border-2 border-white/20 bg-white/5 backdrop-blur-sm flex items-center justify-center">
                  <div className="w-44 h-44">{slide.icon}</div>
                </div>
                ...
                <div className="absolute -top-4 -right-4 bg-white rounded-2xl px-4 py-2.5 shadow-xl">
                  ...
                  <div className="text-gray-500 text-xs mt-0.5">{slide.stats[0].label}</div>
                </div>
