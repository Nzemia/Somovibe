        ...
        <div className="flex items-center gap-1 p-1 bg-gray-100 rounded-xl shrink-0">
          {STATUS_OPTIONS.map((s) => (
            <button
              ...
              className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-bold transition-all duration-150 ${statusFilter === s.value
                  ...
                  <tr key={m.id} className="hover:bg-[#f8fdfb] transition-colors">
                    ...
                    <td className="px-4 py-3.5">
                      <div className="flex items-center gap-2 justify-end">
                        ...
                        <Link
                          href={`/teacher/material/edit/${m.id}`}
                          className="p-1.5 text-gray-400 hover:text-[#008c43] hover:bg-[#f0faf5] rounded-lg transition-colors"
                          title="Edit Material"
                        >
          ...
          <div className="sm:hidden bg-white rounded-2xl overflow-hidden border border-[#e8f5ee] divide-y divide-[#f0faf5]">
            {paged.map((m) => (
              <div key={m.id} className="px-4 py-3.5 flex items-start gap-3">
                {/* Status dot */}
                <span className={`mt-1.5 w-2 h-2 rounded-full shrink-0 ${m.status === "APPROVED" ? "bg-emerald-500" :
              ...
              .map((p, i) =>
                p === "…" ? (
                  ...
                  <button
                    ...
                    className={`w-7 h-7 rounded-lg text-xs font-bold transition-colors ${safePage === p ? "bg-[#008c43] text-white" : "text-gray-600 hover:bg-[#f0faf5] hover:text-[#008c43]"
                      }`}
                  >
                    {p}
                  </button>
